...
public class LinguisticSummarizationsExecutor {
    ...
    public static List<Summary> getSummaries(List<AbsoluteQuantifier> quantifiers, List<LabeledFuzzySet> fuzzySets, List<String> summarizerVariableNames) {
        List<Summary> summaries = new ArrayList<>();

        //Single subject type 1 summaries
        for (AbsoluteQuantifier absoluteQuantifier : quantifiers) {
            List<List<Integer>> sublists = getSublistsOfList(fuzzySets.size());
            summaries.addAll(sublists.stream().map(l -> new SingleType1Summary(absoluteQuantifier,
        ...
        for (AbsoluteQuantifier absoluteQuantifier : quantifiers) {
            ...
            summaries.addAll(sublists.stream().map(l -> {
                ...
                return sublists2.stream().map(l2 -> new SingleType2Summary(absoluteQuantifier,
                        new Subject("wyników", StatsRepository.getStats()),
                        l2.stream().map(fuzzySets::get).toList(),
                        l2.stream().map(summarizerVariableNames::get).toList(),
                        l.stream().map(fuzzySets::get).toList(),
                        l.stream().map(summarizerVariableNames::get).toList(),
                        List.of(0.3, 0.07, 0.07, 0.07, 0.07, 0.07, 0.07, 0.07, 0.07, 0.07, 0.07))).toList();
