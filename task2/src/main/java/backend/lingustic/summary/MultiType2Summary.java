...
public class MultiType2Summary extends MultiSubjectSummary {
    ...
    public MultiType2Summary(AbstractQuantifier quantifier, List<Subject> subjects, List<LabeledFuzzySet> qualifiers, List<String> qualifierVariableNames,
        ...
        super(subjects, summarizers, summarizerVariableNames, quantifier.getLabel() + " " + subjects.get(0).getName() +
                " w porównaniu do tych " + subjects.get(1).getName() + ", które są " + qualifiers.stream().map(LabeledFuzzySet::getLabel).collect(Collectors.joining(" i ")) +
    ...
    protected double evaluateT1() {
        double denominator = (subjects.get(0).getElementsCardinality(summarizers, summarizerVariableNames) / subjects.get(0).getElements().size()) +
                (subjects.get(1).getElementsCardinality(Stream.concat(summarizers.stream(), qualifiers.stream()).toList(),
                        Stream.concat(summarizerVariableNames.stream(), qualifierVariableNames.stream()).toList()) / subjects.get(1).getElements().size());
        if (denominator == 0) {
            return 0;
        }
