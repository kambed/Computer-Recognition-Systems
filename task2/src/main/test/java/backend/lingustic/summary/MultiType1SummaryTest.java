...
class MultiType1SummaryTest {
    ...
    void setUp() {
        ...
        stats.add(new Stats(4, "Alonso Fernando", "Renault", "Albert Park Grand Prix Circuit", "2008", 4, 11, 10, 0.0, 26.6311, 100.00, 88.603, 215.464, null, 86.188, 75, 4.5, -37.8497, 10.0));
        stats.add(new Stats(5, "Kovalainen Heikki", "Mercedes", "Albert Park Grand Prix Circuit", "2008", 5, 3, 30, 0.0, 26.4066, 28.57, 87.418, 218.385, null, 85.452, 75, 4.5, -37.8497, 10.0));
    ...
    @Test
    void evaluateT1Test() {
        Summary s = new MultiType1Summary(
                PredefinedQuantifiers.getPredefinedRelativeQuantifiers().get(2),
                subjects,
                List.of(PredefinedVariables.getPredefinedVariables().get(0).getLabeledFuzzySets().get(0)),
                List.of("Liczba przejechanych okrążeń")
        );
        assertEquals(0.11, s.getT1(), 0.01);
        s = new MultiType1Summary(
                PredefinedQuantifiers.getPredefinedRelativeQuantifiers().get(1),
                subjects,
                List.of(PredefinedVariables.getPredefinedVariables().get(0).getLabeledFuzzySets().get(0)),
                List.of("Liczba przejechanych okrążeń")
        );
        assertEquals(0.66, s.getT1(), 0.01);
    }
